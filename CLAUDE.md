...
- **No import cycles.** `rollup.config.mjs` throws on `CIRCULAR_DEPENDENCY`, so a cycle fails `pnpm bundle`. `signal-core.ts` is the leaf layer (`signalImpl`, `isSignal`, `writeSignal`, `destroySignal`, the signal counter) and must never import `create-signal.ts`, `Signal.ts`, `SignalGroup.ts` or `effects.ts`. The direction has a gate of its own: `scripts/check-layering.mjs` (in `pnpm check`) ranks every library module in a written-down ladder and fails a value import that reaches sideways or up — a new file in `src/` needs a rank there before anything else works. Details in `AGENTS.md` → "Module layering".
...
- Tests are `*.spec.ts` adjacent to the implementation. The `unit` project matches `src/**/*.{spec,test}.ts` plus `scripts/**/*.spec.mjs` — the guard scripts are tested where they live, by spawning them against a fixture tree.
