...
| `thenable-guard.ts` | Leaf, imports nothing — the structural `isThenable` predicate and the `throwIfThenable` guard behind the `TypeError` that `batch()`, `beQuiet()` and `hibernate()` throw on a thenable callback; read by `EffectImpl.ts` for the predicate alone. Not re-exported from either entry point |
...
- `thenable-guard.ts` is a leaf and imports nothing, not even from this package. Its consumers are `batch.ts`, `be-quiet.ts`, `hibernate.ts` and `EffectImpl.ts`, and `EffectImpl.ts` imports `batch.ts` — so any import from here back up would close a ring that `pnpm bundle` rejects and `tsc` waves through.
...
**The direction has its own gate.** `rollup.config.mjs` sees a ring, not an order: an edge that points sideways or upwards without closing one builds cleanly, and the eventual `CIRCULAR_DEPENDENCY` then names whichever edge completed the ring rather than the one that broke the layering. `scripts/check-layering.mjs` runs in `pnpm check` and holds the order itself. It ranks the modules of `tsconfig.lib.json`'s scope — every `src/**/*.ts` that is not a spec and not under `src/__testing__/` — in a ladder of nine layers, and a value import may only reach a strictly lower rank; a sibling edge fails too, which is what makes one a deliberate decision. **That table is the authoritative form of the order**; the rules above say why the edges sit where they do. Type-only imports are outside its reach by design — they carry nothing into the bundle, and `style/useImportType` already forces the marking. A new file in `src/` has no rank, and the guard refuses the tree until the table gives it one.

Also avoid reading an imported binding at module-eval time across module boundaries (`export const x = SomeClass.method`). Delegate through a function instead — `effects.ts:createEffect` is the pattern. An eager read inside a cycle is what would make `import('./lib/EffectImpl.js')` crash with a TDZ `ReferenceError`.
...
| `pnpm check:dts` | `tsc --noEmit --ignoreConfig --skipLibCheck false … lib/**/*.d.ts` (`scripts/check-dts.mjs`) — the one gate that reads the emitted declarations with `skipLibCheck` off, so a `@internal` marker that strips a symbol another `.d.ts` still names is caught as `TS2305`/`TS2304` instead of passing `pnpm check`, `pnpm test:smoke` and `pnpm checkPkgTypes` unnoticed. Not part of `check` — that composite runs before `compile` in `pnpm world` (right after `clean`, which deletes `lib/`), so a `tsc` pass over `lib/**/*.d.ts` there would either find nothing or check stale output. Runs instead right after `compile` in `pnpm world` and right after `pnpm dist` in `ci.yml`, where `lib/` is guaranteed current. Exits 1 with a named reason if `lib/**/*.d.ts` matches no file — a `tsc` invocation with zero files checks nothing, the same silent gap this gate exists to close — and likewise if the `tsc` binary cannot be spawned at all, which produces no diagnostics of its own. `scripts/check-dts.spec.mjs` pins all of it against a fixture `lib/`, over the `CHECK_DTS_ROOT` seam |
| `pnpm lint` | Biome lint only |
| `pnpm format` / `pnpm format:write` | Biome format check / auto-fix |
| `pnpm checkPkgTypes` | `attw --pack --profile esm-only` — package types audit. The profile ignores `node10` and `node16 (from CJS)`, which cannot pass for this ESM-only package (no `exports`-map support / ESM served to a CJS resolver, respectively); `node16 (from ESM)` and `bundler` are still checked in full. Blocks in CI (`pnpm world`, `ci.yml`), not just documented |
| `pnpm dist` | clean + compile + bundle (no test) |

`package.json#files` is an allowlist, not a denylist — there is no `.npmignore`. What ships in the npm tarball: `dist/`, `lib/**/*.d.ts`, `docs/`, `skills/`, plus `README.md`, `CHANGELOG.md`, `LICENSE` and `package.json` — 48 files (2026-08-11). The **file count** is worth keeping here because it only moves when `package.json#files` itself changes; the **kB size** is deliberately not, because `docs/`, `skills/`, `README.md` and `CHANGELOG.md` all ship in the tarball and all get edited by ordinary documentation work — a throwaway ten-line addition to `README.md` alone moved the measured size by ~0.1-0.2 kB, so a number recorded here would already be stale by the next doc package (measured: one documentation package's edits moved it from 618.4/171.1 kB to 621.2/172.1 kB between two `npm pack --dry-run` runs taken hours apart). `npm pack --dry-run` is the way to check both against the current tree.
...
| New utility function | `src/<name>.ts` → **give it a rank in the ladder in `scripts/check-layering.mjs`**, it decides what the module may import → re-export in `src/index.ts` → adjacent `<name>.spec.ts` |
