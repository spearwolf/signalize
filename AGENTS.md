...
| `deprecation-warnings.ts` | One layer above `signalize-error.ts` — the `@internal` once-per-process deprecation gate (`warnDeprecatedOnce`, `DeprecationKey`), read by `create-signal.ts` and `SignalGroup.ts`; not re-exported from either entry point (CONS-004) |
...
- `create-signal.ts` sits above and may reach up to `Signal.ts`, `SignalGroup.ts` and `effect-hook.ts`. **Not `effects.ts`** — neither it nor `Signal.ts` may import that module again; both go through the hook (ARCH-002), and an import edge back would restore the 6 548 bytes that removing it saved.
