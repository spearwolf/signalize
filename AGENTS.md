...
- **Why a browser run wouldn't add coverage anyway:** the one thing that could behave differently across engines is GC timing around `WeakRef`/`FinalizationRegistry`, and the 13 tests that exercise it depend on `--expose-gc` (`vitest.config.ts:59`), a flag no portable browser harness provides. A browser smoke test would skip exactly the tests whose answer it could change.
