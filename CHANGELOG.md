...
- `SignalLink.asyncValues()` no longer hands the same value to the same iterator over and over. Each iterator now tracks the propagation it last consumed and waits for the next one instead of being served the retained value again (ASYNC-005)
- A `for await` over `asyncValues()` without a `stopAction` terminates. It used to spin as a microtask hot loop that starved every timer in the process — measured: 500 000 iterations of one value, not a single macrotask getting through (ASYNC-005)
- A value that arrives while an `asyncValues()` consumer is busy between two reads is still delivered, and several iterators still share one retained slot (ASYNC-005)
- `SignalLink.nextValue()` called on its own is unchanged — it still settles on whatever sits in the retained slot (ASYNC-005)
...
- The same rollback covers a throwing `onCreateEffect()` handler, which sits two lines ahead of the first run and left exactly the same behind (audit follow-up, package 7b)
...
- Reading a memo inside a `batch()` returns its current value instead of the one from before, for a memo the batch itself marked dirty. The memo's recompute used to be deferred like any other run, so a memo whose dependency was written in the same batch read stale — and one *created* in the batch read `undefined`, having no previous value at all. The recompute now happens at the read; its write stays inside the batch and is deduplicated there as before. A memo that is stale only *through another memo* is not reached: nothing marked it dirty, because its upstream's write is the thing the batch is holding back. It still reads its pre-batch value inside the batch, and catches up at the flush if that upstream is eager — behind a `{lazy: true}` upstream it stays on the old value past the flush too, since a lazy memo never pushes. Reading the upstream memo first, inside the same batch, makes the downstream one read fresh in both cases (ASYNC-003, audit 2026-08-08)
...
- `createMemo()`'s recompute no longer wraps `si.set(callback())` in `batch()` by default. If `callback` itself writes to *other* signals as a side effect (uncommon), those writes used to be grouped with the memo's own write so a dependent effect tracking both saw one deduplicated run — without the batch it now sees one run per write, with a torn intermediate value on the first of the two. Pass `{batchWrites: true}` to restore that grouping; the default leaves its cost unpaid for every memo that does not need it — a full batch flush per recompute, once the memo has a dependent effect (see the Performance entries, PERF-002). The read-consistency argument this entry originally also made — that an unconditional `batch()` turned a dirty composed memo's read stale, permanently for a `{lazy: true}` one — no longer applies to either setting, and was superseded within this same release by the ASYNC-003 fix below (PERF-001)
...
- `SignalGroup#attachEffect(effect)` throws `Cannot attach a destroyed effect to a group` where it used to accept an already-destroyed effect silently and hold it until the next `clear()` — the same rule `attachSignal()` and `attachLink()` already applied (CONS-006, audit 2026-08-08)
...
- The type-parameter defaults of the published types are `unknown` instead of `any` (`SignalLike`, `ISignalImpl`, `SignalLink`, `ValueCallback`, `SignalGroup#signal()`); whoever annotated one of them without a type argument now names it — `SignalLike<number>` instead of `SignalLike`; where the annotation genuinely means "some signal, any value type" — a heterogeneous `SignalLike[]`, or a parameter of one's own wrapper — the replacement is `SignalLike<any>`, not `SignalLike<unknown>`: none of these type parameters widens to `unknown` — invariant in the signal and link types, contravariant in `ValueCallback` — so only `any` fits every value type (TYPE-001, audit 2026-08-08)
