...
- `batch(callback)`'s signature now rejects an `async` callback (or anything else typed to return `Promise`/`PromiseLike`) at `tsc` time, and throws `TypeError` at runtime if `callback` returns a thenable. Code that passed an `async` callback previously compiled and appeared to work — it silently stopped batching at the first `await` — and now fails both to compile and, if the type error is ignored or the callback is untyped, at runtime. For the migration: the refused `async` callback has already started, and nothing observes the promise it returned — `batch()` returns `void`, so that promise was never anyone's to await, and a rejection inside it arrives as an unhandled one that ends the process under Node's default. Move the async work out of the frame instead of catching the `TypeError` (ASYNC-003, 2026-07 audit)
- `beQuiet(action)` returns what `action` returns instead of `void`, and its signature rejects an `async` action (or anything else typed to return `Promise`/`PromiseLike`) at `tsc` time — the same narrowing `batch()` got, for the same reason: the quiet frame is closed by its `finally` the moment an `async` action hands back its pending promise at the first `await`, so every read and write from there on is tracked and loud again. A duck-typed thenable from an untyped caller is refused at runtime too, see the entry below; do the awaiting outside and pass a synchronous action. Callers that ignored the return value are unaffected by the widened return type (BUG-010)
...
- `SignalLink#destroy()` now collects a throwing `off(this)` the same way instead of letting it escape past the steps still to run, so the link still ends up destroyed, frozen and with `lastValue` cleared even when that step fails
...
- `SignalGroup#clear()` now collects a throwing `off(this)` instead of letting it escape and abort the rest of the teardown, so the group is still fully torn down and every collected error is reported together
...
- `beQuiet()`'s signature also — like `batch()`'s — rejects an `async`/thenable-returning `action` at compile time: the quiet frame ends at the first `await`, and every read and write after that point is tracked and loud again. A thenable returned at runtime throws `TypeError`, as in `batch()` — see the Breaking Changes entry (BUG-010)
...
- `hibernate(callback)` runs `callback` even when the flush of the saved batch throws — the flush error is held and reported once the batch, quiet counter and effect stack are restored, instead of skipping the callback and handing the caller an error that looks like it came from its own code
...
- New `docs/diamond-example.md` — the diamond problem in the terms of [Reactive algorithms](https://github.com/milomg/reactively/blob/main/Reactive-algorithms.md): why the equality check costs signalize nothing, why a converging effect runs once per path with the first run on a half-updated graph, what `batch()` fixes and what it does not, and the single-memo funnel that gives one run. Linked from `README.md`, `docs/quickstart.md`, `docs/architecture.md` and `docs/recipes.md`, and carried self-contained as pitfall 13b in `skills/using-signalize/references/pitfalls.md`; the top-level `diamond-example.mjs` scratch script it replaces is gone, its behaviour now pinned by `src/diamond-problem.spec.ts`
- `skills/using-signalize/README.md` no longer points out of the skill folder with a relative path. The skill is installed on its own, so `../../docs` resolves to nothing on a reader's machine — the repository is named and linked by URL instead, and the rule is stated on the page
...
- `docs/architecture.md` gained an "Architecture decisions" register: sixteen decisions in context → decision → consequence form, covering synchronous propagation, the eventize queues, symbol namespacing, multi-copy detection, the effect-hook placeholder, top-level side effects, the named export surface, weak lifetimes, eventually consistent counters, collecting teardown, the diagnostics channel, notice frequency, isolated delivery, declarations that resolve under `"lib": ["ES2023"]` alone, the absent browser run and the reproducible banner
...
- The JSDoc of every exported symbol now carries what a caller needs and nothing else — purpose, the conditions under which the symbol surprises, what it returns or throws — while the measured edge cases, params shapes and inference outcomes it used to spell out live in `docs/api.md` and are linked from there. `tsconfig.types.json` sets `removeComments: false`, so this is the shipped surface — `lib/*.d.ts` and with it every consumer's editor tooltip shrank accordingly
- Two JSDoc promises that were never true are corrected: `EffectOptionsWithNameDeps` claimed to prevent a `TypeError` that does not exist — `EffectImpl` refuses a group-less name lookup itself, with a message — and `createEffect()` named the immediate first run as the default, which static `dependencies` switch off no matter what `autorun` says
