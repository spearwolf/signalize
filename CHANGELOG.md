...
- `getSignalsCount()` now counts reachable signals instead of created ones, and settles eventually rather than immediately: a signal dropped without `destroySignal()` is subtracted only once the garbage collector reclaims it, at a moment that cannot be observed or forced. Code that reads the count right after a teardown as a deterministic leak assertion — `src/assert-helpers.ts` does exactly this — can see a stale, too-high number even though nothing actually leaked. Poll the count (or force a collection with `--expose-gc`) instead of reading it once (MEM-006, audit 2026-08-07)
